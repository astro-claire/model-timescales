...
def relaxation_timescale(v,rho, mass, coulomb=10, v_unit = u.km/u.s, rho_unit = u.g/u.cm**3,mass_unit = u.Msun, mass_spectrum_psi = 1.0):
    
    v = as_quantity(v,v_unit)
    rho = as_quantity(rho, rho_unit)
    mass = as_quantity(mass, mass_unit)

    return (0.34 * v**3 / (c.G**2 * rho * mass *coulomb * mass_spectrum_psi)).to('yr')
