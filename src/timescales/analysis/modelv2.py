    ...
    for sys_id in range(ensemble.Nsystems):
        ...
        if "BH" in ensemble.densityModel:
            ...
            if r_ml>rmin:
                ...
                #######CHOOSING SOME INTERESTING RADII
                ncol = 0
                if r_ml >0.01 *u.pc: 
                    ncol = N_coll_bh_limits(prof.r0,
                                        ...
                                        rmax =0.01 *u.pc,
                                        e = ensemble.timescales_kwargs["e"])
                    mass_encl1= prof.enclosed_mass(0.01 *u.pc)
                ...
                if r_ml >0.001 *u.pc: 
                    ncol = N_coll_bh_limits(prof.r0,
                                        ...
                                        rmax =0.001 *u.pc,
                                        e = ensemble.timescales_kwargs["e"])
                    mass_encl1= prof.enclosed_mass(0.001 *u.pc)
                    if ncol *u.Msun >mass_encl1:
                        masscoll = 0.5*mass_encl1
                    else:
                        masscoll = ncol *u.Msun
                ...
                if r_ml >1. *u.pc: 
                    ncol = N_coll_bh_limits(prof.r0,
                                        ...
                                        rmax =1. *u.pc,
                                        e = ensemble.timescales_kwargs["e"])
                    mass_encl1= prof.enclosed_mass(1 *u.pc)
                    if ncol *u.Msun >mass_encl1:
                        masscoll = 0.5*mass_encl1
                    else:
                        masscoll = ncol *u.Msun
                    ...
                    masscoll = 0.0*u.Msun
                out['rhotot_1e0pc'][sys_id]=masscoll / (4*np.pi/3.*(1. *u.pc)**3)
            ...
            out['N_collisions_df'][sys_id] = Ncoll_pl_no_bh_limits(prof.r0,
                                        ...
                                        rmin =rmin,
                                        e = ensemble.timescales_kwargs["e"])
            out['mass_df_rate'][sys_id] =  Mdot_pl_no_bh_limits(prof.r0,
                                        newts, 
                                        prof.alpha, 
                                        cv,
                                        prof.rho0,
